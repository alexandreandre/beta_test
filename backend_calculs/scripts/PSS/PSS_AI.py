...
def get_plafonds_ss_via_ai() -> dict | None:
    ...
    Les clés doivent être en minuscule, basées sur le mapping suivant : Année->annuel, Trimestre->trimestriel, Mois->mensuel, Quinzaine->quinzaine, Semaine->hebdomadaire, Jour->journalier, Heure->horaire.
...
def main():
    ...
    plafonds_data = get_plafonds_ss_via_ai()
    
    if not plafonds_data:
        print("ERREUR CRITIQUE: Le scraping des plafonds via IA a échoué.", file=sys.stderr)
        sys.exit(1)

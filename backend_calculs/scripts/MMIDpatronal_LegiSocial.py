# scripts/MMIDpatronal_LegiSocial.py
...
def parse_taux(text: str) -> float | None:
    """
    Nettoie un texte (ex: "13,00 %"), le convertit en float (13.00)
    puis en taux réel (0.1300).
    """
    if not text:
        return None
    try:
...
def get_taux_maladie_legisocial() -> dict | None:
    """
    Scrape le site LegiSocial pour trouver les taux plein et réduit de la cotisation maladie patronale.
    ...
    try:
        ...
        for row in table.find('tbody').find_all('tr'):
            ...
            if len(cells) > 4:
                ...
                # On cherche "maladie" mais on exclut "alsace-moselle"
                if "maladie" in libelle and "alsace-moselle" not in libelle:
                    ...
                    if taux is not None:
                        ...
                        if "≤" in libelle or "<" in libelle:
                            print(f"Taux Maladie Patronal (réduit) trouvé : {taux*100:.2f}%")
                            taux_trouves['reduit'] = taux
                        elif ">" in libelle:
                            print(f"Taux Maladie Patronal (plein) trouvé : {taux*100:.2f}%")
                            taux_trouves['plein'] = taux
        
        if "reduit" in taux_trouves and "plein" in taux_trouves:
            return taux_trouves
        else:
            raise ValueError("Impossible de trouver les deux taux maladie (réduit et plein).")
...
def update_config_file(nouveau_taux: float):
    """
    Met à jour le fichier de configuration avec le taux maladie applicable.
    """
    try:
...
if __name__ == "__main__":
    tous_les_taux = get_taux_maladie_legisocial()
    ...
    if tous_les_taux:
        try:
            ...
            # Note: la condition pour le taux réduit maladie et allocations familiales est la même dans notre projet
            ...
            if appliquer_taux_reduit:
                ...
                print(f"\nCondition de bas salaire active. Application du taux réduit : {taux_final*100:.2f}%")
            else:
                taux_final = tous_les_taux['plein']
                print(f"\nCondition de haut salaire active. Application du taux plein : {taux_final*100:.2f}%")
